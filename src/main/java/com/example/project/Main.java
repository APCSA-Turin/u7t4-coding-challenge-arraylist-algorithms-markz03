...
public class Main{
   ...
   */
    public static ArrayList<Integer> reverseArray(int[] intList){
        ...
        ArrayList<Integer> modifiedIntList = new ArrayList<>();

        for (int i = 0; i < intList.length; i++) {
            modifiedIntList.add(0, intList[i]);
        }
    ...
    public static ArrayList<String> parseSentence(String sentence){
        ...
        for (int i = 0; i < sentence.length() - 1; i++) {
            if (sentence.indexOf(" ") != -1) {
                strList.add(sentence.substring(0, sentence.indexOf(" ")));
    ...
    public static ArrayList<String> moveBWords(ArrayList<String> wordList){
        ...
        for (int i = 0; i < wordList.size(); i++) {
            if (wordList.get(i).substring(0, 1).equals("b") || wordList.get(i).substring(0, 1).equals("B")) {
                wordList.add(arrayIdx, wordList.get(i));
                arrayIdx++;
                wordList.remove(i + 1);
            }
        }
   ...
   */
    public static ArrayList<Integer> modes(int[] numList){
        ...
        ArrayList<Integer> amountCheck = new ArrayList<>();
        ...
        for (int i = 0; i < numList.length - 1; i++) {
            ...
            amountCheck.add(count);
            removeDuplicates(amountCheck);
            if (count > max) {
                max = count;
